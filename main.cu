...
int main(int argc, char **argv){
    ...
    // kernel
    if(alg == 0){
        printf("\033[32;1m[matmul basico]\033[0m.............");
        ...
        printf("\033[32;1m[matmul tc 16x16 blocks]\033[0m.......");
        block = dim3(TCSIZE, TCSIZE, 1);    
        grid = dim3((totaln+8*TCSQ-1)/(8*TCSQ), 1, 1);    
        timer_start(&start, &stop);
        matmuls_tc_block<<<grid, block>>>(Adh, Bdh, Cd, totaln);
    }
    else if(alg == 4){
        printf("\033[32;1m[matmul tc 16x16 blocks-sm]\033[0m.......");
        block = dim3(TCSIZE, TCSIZE, 1);    
        grid = dim3((totaln+8*TCSQ-1)/(8*TCSQ), 1, 1);    
        timer_start(&start, &stop);
